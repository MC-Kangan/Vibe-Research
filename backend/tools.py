...
TOOLS: list[dict] = [
    ...
    _t("query_market_bars",
       ...
       ["symbol"]),
    _t("query_crypto_snapshot",
       "查 Coinbase USD 现货加密货币行情快照，如 BTC、ETH、SOL。",
       {"symbol": {"type": "string", "description": "加密货币代码，如 BTC 或 BTC-USD"}}, ["symbol"]),
    _t("query_crypto_bars",
       "查 Coinbase 加密货币 UTC 日线 OHLCV 历史。",
       {"symbol": {"type": "string", "description": "加密货币代码，如 BTC"},
        "range": {"type": "string", "enum": ["1mo", "3mo", "6mo", "1y", "2y"]}}, ["symbol"]),
    _t("query_crypto_context",
...
_HANDLERS = {
    ...
    "query_crypto_snapshot": lambda a: asdict(market_data.get_snapshot(str(a.get("symbol", "")), "crypto")),
    "query_crypto_bars": lambda a: asdict(market_data.get_bars(str(a.get("symbol", "")), str(a.get("range") or "1y"), "1d", "crypto")),
    "query_crypto_context": lambda a: market_data.get_crypto_asset_context(str(a.get("symbol", ""))),
