...
def resolve_research_identity(symbol: str, asset_type: str) -> tuple[str, str]:
    """Return the provider symbol and TradeAgent market for a supported instrument."""
    value = (symbol or "").strip().upper()
    if asset_type == "crypto":
        return market_data.resolve_crypto_symbol(value), "CRYPTO"
    ...
    return resolved.provider_symbol, "EU" if resolved.exchange.country != "US" else "US"
...
def _a_share_reports(symbol: str) -> list[dict[str, Any]]:
    ...
    for row in rows:
        row["pdfUrl"] = (
            astock.pdf_url(row.get("infoCode", "")) if row.get("infoCode") else None
        )
    return rows
...
def _run(
    ...
    with ThreadPoolExecutor(max_workers=min(6, len(tasks))) as executor:
        futures = {
            executor.submit(loader): section for section, loader in tasks.items()
        }
...
def get_overview(symbol: str, asset_type: str = "equity") -> dict[str, Any]:
    ...
    if asset_type == "crypto":
        ...
        data, gaps = _run(
            {
                "market_snapshot": lambda: market_data.get_snapshot(
                    canonical, "crypto"
                ),
                "market_history": lambda: market_data.get_bars(
                    canonical, "1y", "1d", "crypto"
                ),
            }
        )
        return _response(
            symbol=canonical.removesuffix("-USD"),
            route="crypto",
            data=data,
            gaps=gaps,
            primary=("market_snapshot", "market_history"),
            capabilities=["quote", "history", "skills"],
        )
    if asset_type != "equity":
        raise market_data.UnsupportedSymbolError("asset_type must be equity or crypto")
        ...
        data, gaps = _run(
            {
                ...
                **(
                    {"cashflow": lambda: gstock.hk_cashflow(value)}
