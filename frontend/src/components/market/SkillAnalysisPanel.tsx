...
const PERCENT_METRICS = new Set([
  ...
  "volatility_regime_percentile", "on_balance_volume_trend_20",
]);

function metricLabel(metric: string): string {
  return metric.split("_").map((part) => part.toUpperCase() === "ADX" || part.toUpperCase() === "EMA" ? part.toUpperCase() : part).join(" ");
}
...
function WorthBuyReport({ result }: { result: Json }) {
  ...
  const priceOption = useMemo(() => ({
    ...
    series: [{
      ...
      tooltip: { valueFormatter: formatCandlestickTooltip },
  ...
  const scoreOption = useMemo(() => ({
    ...
    xAxis: { type: "value", max: 100, axisLabel: { color: "#94a3b8", formatter: (value: number) => value.toFixed(2) } },
    yAxis: { type: "category", data: components.map((item: Json) => item.key), axisLabel: { color: "#cbd5e1" } },
    series: [{ type: "bar", data: components.map((item: Json) => item.score), tooltip: { valueFormatter: (value: unknown) => formatSkillNumber(value) }, itemStyle: { color: "#f97316" }, label: { show: true, position: "right", color: "#cbd5e1", formatter: (item: Json) => formatSkillNumber(item.value) } }],
  }), [components]);
  return <div className="space-y-4">
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      <div className="rounded-lg bg-muted/30 p-3"><p className="text-xs text-muted-foreground">{tr("Composite", "综合得分")}</p><p className="font-mono text-lg font-bold">{formatSkillNumber(values.worth_buy_composite)}</p></div>
      <div className="rounded-lg bg-muted/30 p-3"><p className="text-xs text-muted-foreground">{tr("Risk veto", "风险否决")}</p><p className="font-mono text-lg font-bold">{formatSkillNumber(values.worth_buy_risk_veto)}</p></div>
...
function MarkovReport({ result }: { result: Json }) {
  ...
  const option = useMemo(() => {
    ...
    return {
      ...
      yAxis: [
        { scale: true, axisLabel: { color: "#94a3b8", formatter: (value: number) => formatSkillNumber(value) }, splitLine: { lineStyle: { color: "rgba(148,163,184,.12)" } } },
        { gridIndex: 1, axisLabel: { color: "#94a3b8", formatter: (value: number) => `${value.toFixed(2)}%` }, splitLine: { lineStyle: { color: "rgba(148,163,184,.12)" } } },
      ...
      series: [
        ...
        { name: "Rolling return", type: "line", xAxisIndex: 1, yAxisIndex: 1, showSymbol: false, data: points.map((item: Json) => Number(item.rolling_return) * 100), tooltip: { valueFormatter: (value: unknown) => `${formatSkillNumber(value)}%` }, lineStyle: { color: "#c084fc" }, markLine: { symbol: "none", data: [{ yAxis: Number(presentation.bull_threshold ?? 0.05) * 100, lineStyle: { color: "#22c55e", type: "dashed" }, label: { formatter: "Bull" } }, { yAxis: Number(presentation.bear_threshold ?? -0.05) * 100, lineStyle: { color: "#ef4444", type: "dashed" }, label: { formatter: "Bear" } }] } },
        { name: "Regime", type: "bar", xAxisIndex: 2, yAxisIndex: 2, barWidth: "100%", tooltip: { valueFormatter: (value: unknown) => ["Bear", "Sideways", "Bull"][Number(value)] || "—" }, data: points.map((item: Json) => ({ value: item.regime === "Bull" ? 2 : item.regime === "Bear" ? 0 : 1, itemStyle: { color: regimeColors[item.regime] || "#64748b" } })) },
      ],
    };
  }, [points, presentation.bull_threshold, presentation.bear_threshold]);
  const stationaryOption = useMemo(() => ({ grid: { left: 92, right: 20, top: 18, bottom: 28 }, xAxis: { type: "value", max: 1, axisLabel: { color: "#94a3b8", formatter: (v: number) => formatSkillPercent(v) } }, yAxis: { type: "category", data: ["Bear", "Sideways", "Bull"], axisLabel: { color: "#cbd5e1" } }, series: [{ type: "bar", data: stationary, itemStyle: { color: (p: Json) => ["#ef4444", "#f59e0b", "#22c55e"][p.dataIndex] }, label: { show: true, position: "right", formatter: (p: Json) => formatSkillPercent(p.value), color: "#cbd5e1" } }] }), [stationary]);
  const persistenceOption = useMemo(() => ({ grid: { left: 130, right: 20, top: 18, bottom: 28 }, xAxis: { type: "value", max: 1, axisLabel: { color: "#94a3b8", formatter: (value: number) => formatSkillNumber(value) } }, yAxis: { type: "category", data: ["Bear Persistence", "Sideways Persistence", "Bull Persistence"], axisLabel: { color: "#cbd5e1" } }, series: [{ type: "bar", data: persistence, itemStyle: { color: "#64748b" }, label: { show: true, position: "right", color: "#cbd5e1", formatter: (item: Json) => formatSkillNumber(item.value) } }] }), [persistence]);
  const regimeLabel = locale === "en" ? regime : (REGIME_ZH[regime] || regime);
  return <div className="space-y-4">
    <div className={`rounded-xl border p-4 ${regime === "Bull" ? "border-success/30 bg-success/5" : regime === "Bear" ? "border-danger/30 bg-danger/5" : "border-warning/30 bg-warning/5"}`}><p className="text-lg font-semibold">{tr("Current regime: ", "当前状态：")}{regimeLabel}</p><p className="mt-1 text-sm text-muted-foreground">{result.summary || tr("Markov regime report completed.", "Markov 状态报告已完成。")}</p><p className="mt-2 text-xs text-muted-foreground">{tr("Window", "窗口")} {presentation.window ?? 20}d · {tr("Bull", "多头")} ≥ {formatSkillPercent(presentation.bull_threshold ?? 0.05)} · {tr("Bear", "空头")} ≤ {formatSkillPercent(presentation.bear_threshold ?? -0.05)}</p></div>
    ...
    {showStationary && <div className="overflow-x-auto rounded-lg border border-border/60"><table className="w-full text-left text-xs"><thead className="bg-muted/30 text-muted-foreground"><tr><th className="p-2">{tr("Stationary distribution", "稳态分布")}</th><th className="p-2">{tr("Probability", "概率")}</th><th className="p-2">{tr("Persistence", "持续性")}</th></tr></thead><tbody>{["Bear", "Sideways", "Bull"].map((name, index) => <tr key={name} className="border-t border-border/40"><td className="p-2">{locale === "en" ? name : REGIME_ZH[name]}</td><td className="p-2 font-mono">{formatSkillPercent(stationary[index])}</td><td className="p-2 font-mono">{formatSkillPercent(persistence[index])}</td></tr>)}</tbody></table></div>}
    <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-muted/30 p-3"><p className="text-xs text-muted-foreground">{tr("Directional signal", "方向信号")}</p><p className="font-mono text-lg font-bold">{formatSkillNumber(values.markov_signal)}</p></div><div className="rounded-lg bg-muted/30 p-3"><p className="text-xs text-muted-foreground">{tr("Observed points", "观测点数")}</p><p className="text-sm">{points.length || "—"}</p></div><div className="rounded-lg bg-muted/30 p-3"><p className="text-xs text-muted-foreground">{tr("Data quality", "数据质量")}</p><p className="text-sm">{result.status || tr("unknown", "未知")}</p></div></div>
...
function PriceSeriesReport({ result }: { result: Json }) {
  ...
  return <div className="space-y-4">
    <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
      <p className="text-lg font-semibold capitalize">{result.analyst?.split("-").join(" ")}</p>
      <p className="mt-1 text-sm text-muted-foreground">{result.summary}</p>
      <p className="mt-2 text-xs text-muted-foreground">{tr("Signal", "信号")}: <span className="capitalize text-foreground">{signal}</span> · {tr("Daily price-series analysis", "日线价格序列分析")}</p>
    </div>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item: Json) => <div key={item.metric} className="rounded-lg border border-border/60 p-3">
        <p className="text-xs capitalize text-muted-foreground">{metricLabel(item.metric)}</p>
        <p className="mt-1 font-mono text-lg font-semibold">{formatMetric(item.metric, item.value)}</p>
      </div>)}
    </div>
