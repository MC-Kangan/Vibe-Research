...
const TOOL_LABEL: Record<string, { en: string; zh: string }> = {
  query_quote: { en: "Quote", zh: "查行情" },
  query_valuation: { en: "Valuation", zh: "查估值" },
  query_reports: { en: "Research", zh: "查研报" },
  query_news: { en: "News", zh: "查新闻" },
};
...
export function AskAiButton({ workflow, context, suggestions = [], label, scopeKey }: Props) {
  const { tr } = useLocale();
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      if (!superseded && chatKeyRef.current === startedKey) {
        ...
        if (!ac.signal.aborted) setErr(e instanceof ApiError ? e.message : tr("Chat failed", "对话失败"));
      }
    } finally {
          ...
          <aside className="glass relative m-3 flex w-full max-w-md flex-col rounded-2xl">
            <div className="flex items-center justify-between border-b border-border/60 p-4">
              <span className="flex items-center gap-2 font-semibold text-glow">
                <Sparkles className="h-4 w-4 text-primary" /> {tr("Ask AI", "问 AI")} · {tr(workflowInfo.label.en, workflowInfo.label.zh)}
              </span>
              <div className="flex items-center gap-1">
                {msgs.length > 0 && (
                  ...
                  <button
                    onClick={clearChat}
                    title={tr("Clear this conversation", "清空本页对话")}
                    aria-label={tr("Clear this conversation", "清空本页对话")}
                    className="text-muted-foreground hover:text-foreground"
                  >
                    <Trash2 className="h-4 w-4" />
                  </button>
                )}
                <button onClick={close} className="text-muted-foreground hover:text-foreground" aria-label={tr("Close", "关闭")}>
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
            ...
            {!configured ? (
              ...
              <div className="flex-1 space-y-4 overflow-auto p-4 text-sm">
                <div className="rounded-lg border border-warning/30 bg-warning/5 p-3 text-xs text-muted-foreground">
                  {tr("Your configured AI produces the analysis. Vibe only packages this page as context and exposes controlled data tools; ", "分析结论由你自己配置的 AI 给出，本产品只负责把本页数据打包成上下文、并让 AI 能调数据工具，")}
                  <b className="text-foreground">{tr("it does not validate or endorse the result", "不校准、不背书、不对结果负责")}</b>.
                </div>
                <div>
                  <p className="mb-1.5 text-xs font-medium text-muted-foreground">{tr("Page context sent with your question:", "将随提问发给 AI 的本页上下文：")}</p>
...
{context}
                ...
                <Link to="/settings" className="flex items-center justify-center gap-2 rounded-lg bg-primary/15 px-3 py-2 text-sm font-medium text-primary hover:bg-primary/25">
                  <Settings className="h-4 w-4" /> {tr("Configure your AI (subscription / API)", "先接入你的 AI（订阅 / API）")}
                </Link>
              </div>
            ) : (
              // 已接入：真对话
                ...
                <div ref={scrollRef} className="flex-1 space-y-3 overflow-auto p-4 text-sm">
                  <div className="rounded-lg border border-border/70 bg-muted/20 p-3 text-[11px] text-muted-foreground">
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                      <span className="inline-flex items-center gap-1 text-foreground"><Database className="h-3 w-3 text-primary" />{runtimeMeta?.mode === "context_only" || (!runtimeMeta && providerIsCli) ? tr("Page context only", "仅页面上下文") : tr("Controlled Vibe data tools", "受控 Vibe 数据工具")}</span>
                      <span className="inline-flex items-center gap-1"><Globe2 className="h-3 w-3" />{tr("General web search disabled", "通用网页搜索未开启")}</span>
                      <span>{tr("Private knowledge not connected", "私有知识库未连接")}</span>
                    </div>
                    <p className="mt-1">{tr(workflowInfo.purpose.en, workflowInfo.purpose.zh)}</p>
                  </div>
                  {msgs.length === 0 && (
                    <div className="rounded-lg border border-warning/30 bg-warning/5 p-3 text-xs text-muted-foreground">
                      {tr("API models can only call the read-only tools allowed for this workflow; CLI models receive only page context. Your model produces the result, which ", "API 模型只能调用此工作流允许的只读工具；CLI 模型只接收本页上下文。结论由你的模型给出，")}
                      <b className="text-foreground">{tr("is not investment advice", "不构成投资建议")}</b>.
                    </div>
                  )}
                  {msgs.map((m, i) => (
                      ...
                      <div className={cn(
                        ...
                        {m.tools && m.tools.length > 0 && (
                          <div className="mb-1.5 flex flex-wrap items-center gap-1">
                            <span className="text-[10px] text-muted-foreground/70">{tr("Data sources", "数据来源")}</span>
                            {m.tools.map((t, j) => (
                              <span key={j} className="inline-flex items-center gap-0.5 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] text-primary">
                                <Wrench className="h-2.5 w-2.5" /> {TOOL_LABEL[t.name] ? tr(TOOL_LABEL[t.name].en, TOOL_LABEL[t.name].zh) : t.name}{t.arg ? ` ${t.arg}` : ""}
                              </span>
                            ))}
                          </div>
                        )}
                        ...
                        {m.role === "assistant" && m.content && !(loading && i === msgs.length - 1) && (
                          <div className="mt-1.5"><SaveNoteButton kind={tr("Ask AI", "问AI")} title={`${tr("Ask AI", "问 AI")} · ${msgs[i - 1]?.content?.slice(0, 24) || tr("Conversation", "对话")}`} content={m.content} /></div>
                        )}
                      </div>
                    </div>
                  ))}
                  {loading && (
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <Loader2 className="h-3.5 w-3.5 animate-spin" /> {tr("AI is thinking / retrieving data…", "AI 正在思考 / 调取数据…")}
                    </div>
                  )}
                  {err && (
                ...
                <div className="border-t border-border/60 p-3">
                  <div className="flex items-end gap-2">
                    <textarea
                      ...
                      rows={1}
                      placeholder={tr("Ask about this page…", "就本页内容提问…")}
